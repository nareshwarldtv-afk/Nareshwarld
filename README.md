...
<head>
    ...
    <title>Naresh warld Admin Panel</title>
    <link rel="preconnect" href="https://fonts.googleapis.com">
    <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
    <link href="https://fonts.googleapis.com/css2?family=Poppins:wght@400;500;600;700&display=swap" rel="stylesheet">
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.2.1/css/all.min.css">
    
    <!-- FIREBASE SCRIPTS (Auth Script Added Here) -->
    <script src="https://www.gstatic.com/firebasejs/9.15.0/firebase-app-compat.js"></script>
    <script src="https://www.gstatic.com/firebasejs/9.15.0/firebase-auth-compat.js"></script> <!-- ADDED THIS -->
    <script src="https://www.gstatic.com/firebasejs/9.15.0/firebase-database-compat.js"></script>
    
    <style>
        :root { --primary-color: #e50914; --primary-hover: #f40612; --gradient: linear-gradient(90deg, #e50914 0%, #b80d14 100%); --bg-main: #000000; --bg-content: #0a0a0a; --bg-input: #1f1f1f; --border-color: #2a2a2a; --text-primary: #ffffff; --text-secondary: #b3b3b3; --sidebar-width: 260px; --success-color: #2e7d32; --warning-color: #f57c00; }
        * { margin: 0; padding: 0; box-sizing: border-box; font-family: 'Poppins', sans-serif; }
        ...
        .stat-card { background-color: var(--bg-content); padding: 25px; border-radius: 12px; display: flex; align-items: center; gap: 20px; border: 1px solid var(--border-color); position: relative; overflow: hidden; }
        ...
        .banned-badge { background: #c62828; color: white; padding: 2px 8px; border-radius: 4px; font-size: 10px; margin-left: 8px; vertical-align: middle; }
        ...
        /* Plan Card Styles */
        .plan-card { background-color: var(--bg-content); border-radius: 8px; padding: 20px; border: 1px solid var(--border-color); position: relative; margin-bottom: 15px;}
        ...
        .premium-check-label { color: #ffb700; font-weight: 600; cursor: pointer; display: flex; align-items: center; gap: 10px; }

        /* ADS SPECIFIC STYLES */
        .ad-card { background: var(--bg-content); border: 1px solid var(--border-color); border-radius: 8px; overflow: hidden; position: relative; display:flex; flex-direction:column; }
        .ad-preview { width: 100%; height: 180px; background: #000; display: flex; align-items: center; justify-content: center; overflow: hidden; position: relative; }
        .ad-preview img { width: 100%; height: 100%; object-fit: cover; }
        .ad-preview video { width: 100%; height: 100%; object-fit: contain; }
        .ad-type-badge { position: absolute; top: 10px; left: 10px; padding: 4px 8px; background: var(--primary-color); color: #fff; font-size: 10px; border-radius: 4px; font-weight: 600; z-index: 10;}
        ...
        .ad-controls { padding: 15px; border-top: 1px solid var(--border-color); display: flex; justify-content: space-between; align-items: center; background: var(--bg-input); }

        /* Redeem Codes Table */
        .code-available { color: #2e7d32; font-weight: 600; }
        .code-used { color: #c62828; font-weight: 600; }
        .code-delete-btn { background: none; border: none; color: var(--text-secondary); font-size: 18px; cursor: pointer; }
...
<body>
    <aside class="sidebar" id="sidebar">
        <div class="sidebar-header"> <h1>Naresh warld</h1> </div>
        <ul class="nav-menu">
    ...
    <main class="main-content">
        ...
        <section id="add-movie-view" class="view">
            ...
            <div class="form-container">
                ...
                <!-- UPDATED: Multiple Category Selection -->
                ...
                <div class="form-group" style="display: flex; gap: 20px;">
                    <label style="cursor: pointer; width: auto;">
                        <input type="checkbox" id="movie-is-new" style="width: auto; height: 16px; margin-right: 10px;"> Mark as 'New'
                    </label>
                    <label class="premium-check-label">
                        <input type="checkbox" id="movie-is-premium" style="width: auto; height: 16px; margin-right: 10px;"> <i class="fa-solid fa-lock"></i> Premium (VIP Only)
                    </label>
                </div>
                <hr>
        ...
        <section id="add-series-view" class="view">
            ...
            <div class="form-container">
                ...
                <div class="form-group">
                 ...
                 <div class="form-group" style="display: flex; gap: 20px;">
                    <label style="cursor: pointer; width: auto;">
                        <input type="checkbox" id="series-is-new" style="width: auto; height: 16px; margin-right: 10px;"> Mark as 'New'
                    </label>
                    <label class="premium-check-label">
                        <input type="checkbox" id="series-is-premium" style="width: auto; height: 16px; margin-right: 10px;"> <i class="fa-solid fa-lock"></i> Premium Series (VIP)
                    </label>
                </div>
                <hr>
        ...
        <!-- NEW REDEEM CODES VIEW (UPDATED WITH PRICE) -->
            ...
            <div class="table-container">
                <table class="styled-table">
                    ...
                    <tbody id="redeem-codes-body">
                        <tr><td colspan="7" style="text-align: center;">Loading codes...</td></tr>
                    </tbody>
                </table>
            </div>
        </section>
        
        <!-- NEW USER BAN SECTION -->
        ...
        <!-- NEW APP CONTROL / VERSION SECTION -->
        <section id="app-control-view" class="view">
            ...
            <div class="form-container">
                ...
                <div class="form-group">
                    <label><i class="fa-solid fa-code-branch"></i> Version Code (Integer)</label>
                    <input type="number" id="app-ver-code" placeholder="e.g., 10">
                </div>
                <div class="form-group">
                    <label><i class="fa-solid fa-tag"></i> Version Name (String)</label>
                    <input type="text" id="app-ver-name" placeholder="e.g., v2.1.0">
                </div>
                <div class="form-group">
                     <label style="cursor: pointer; width: auto;">
                        <input type="checkbox" id="app-force-update" style="width: auto; height: 16px; margin-right: 10px;"> Force Update Required
                    </label>
                </div>
                <div class="form-group">
                    <label><i class="fa-solid fa-link"></i> APK Download URL</label>
                    <input type="text" id="app-apk-url" placeholder="https://.../app-release.apk">
                </div>
                <div class="form-group">
                    <label><i class="fa-solid fa-clipboard-list"></i> What's New (Changelog)</label>
                    <textarea id="app-changelog" placeholder="- Fixed bugs&#10;- Added new features"></textarea>
                </div>
...
  appId: "1:275124357254:web:63cc537b62f76e67f92907"
    ...
    // Auth is initialized by the script tag, we can access it via firebase.auth() if needed
    ...
    function showView(viewId) {
        ...
        else if (viewId.includes('manage-ads')) activeLink = document.getElementById('nav-manage-ads');
        else if (viewId.includes('manage-plans')) activeLink = document.getElementById('nav-manage-plans');
        else if (viewId.includes('payment-settings')) activeLink = document.getElementById('nav-payment-settings');
        else if (viewId.includes('premium-requests')) activeLink = document.getElementById('nav-premium-requests');
        else if (viewId.includes('manual-premium')) activeLink = document.getElementById('nav-manual-premium');
        else if (viewId.includes('redeem-codes')) activeLink = document.getElementById('nav-redeem-codes');
        ...
        else if (viewId.includes('app-control')) activeLink = document.getElementById('nav-app-control');
    ...
    function handleSaveMovie() { 
        ...
        const selectedCategories = Array.from(document.querySelectorAll('#movie-category-wrapper input:checked'))
                                        .map(cb => cb.value)
                                        .join(', ');
        ...
        const movieData = { 
            ...
            cast: Array.from(document.querySelectorAll('#movie-cast-container .cast-item')).map(item => ({ name: item.querySelector('p').textContent, photo: item.querySelector('img').src })), 
        ...
        if (editId) { 
            database.ref('movies/' + editId).update(movieData).then(() => { showStatus('Movie updated!', 'success'); clearMovieForm(); showView('manage-movies-view'); }).catch(error => { showStatus('Update failed', 'error'); console.error(error); }); 
        } else { 
            database.ref('movies').push(movieData).then(() => { showStatus('Movie saved!', 'success'); clearMovieForm(); showView('manage-movies-view'); }).catch(error => { showStatus('Save failed', 'error'); console.error(error); }); 
        } 
    }
    ...
    function editMovie(id) { 
        database.ref('movies/' + id).once('value').then(snapshot => { 
            ...
            if (movie.cast && Array.isArray(movie.cast)) { movie.cast.forEach(actor => addCastMember(actor, 'movie-cast-container')); } 
    ...
    function handleSaveSeries() { 
        ...
        const selectedCategories = Array.from(document.querySelectorAll('#series-category-wrapper input:checked'))
                                        .map(cb => cb.value)
                                        .join(', ');
        ...
        const seriesData = { 
            ...
            cast: Array.from(document.querySelectorAll('#series-cast-container .cast-item')).map(item => ({ name: item.querySelector('p').textContent, photo: item.querySelector('img').src })), 
            episodes: episodes, 
            type: 'webseries', 
            timestamp: Date.now() 
        }; 
        ...
        if (editId) { 
            database.ref('webseries/' + editId).update(seriesData).then(() => { showStatus('Series updated!', 'success'); clearSeriesForm(); showView('manage-series-view'); }).catch(error => showStatus('Update failed: ' + error.message, 'error')); 
        } else { 
            database.ref('webseries').push(seriesData).then(() => { showStatus('Series saved!', 'success'); clearSeriesForm(); showView('manage-series-view'); }).catch(error => showStatus('Save failed: ' + error.message, 'error')); 
        } 
    }
    ...
    function editSeries(id) { 
        database.ref('webseries/' + id).once('value').then(snapshot => { 
            ...
            if (series.cast && Array.isArray(series.cast)) { series.cast.forEach(actor => addCastMember(actor, 'series-cast-container')); } 
            ...
            if (episodes) { 
                const episodesBySeason = episodes.reduce((acc, ep) => { (acc[ep.season] = acc[ep.season] || []).push(ep); return acc; }, {}); 
                for (const seasonNumber in episodesBySeason) { 
                    ...
                    episodesBySeason[seasonNumber].forEach(ep => addEpisode(episodesContainer, ep)); 
                } 
            } 
    ...
    function loadCategories() { 
        ...
        const movieCatWrapper = document.getElementById('movie-category-wrapper');
        const seriesCatWrapper = document.getElementById('series-category-wrapper'); 

        database.ref('categories').orderByChild('name').on('value', snapshot => { 
            if (listContainer) listContainer.innerHTML = ''; 
            if (movieCatWrapper) movieCatWrapper.innerHTML = ''; 
            if (seriesCatWrapper) seriesCatWrapper.innerHTML = ''; 

            const categoryCount = snapshot.numChildren(); 

            if (categoryCount === 0) { 
                if (listContainer) listContainer.innerHTML = '<p style="color: var(--text-secondary);">No categories found.</p>'; 
                const noCatMsg = '<p style="color: var(--text-secondary); font-size: 12px;">No categories available.</p>';
                if (movieCatWrapper) movieCatWrapper.innerHTML = noCatMsg;
                if (seriesCatWrapper) seriesCatWrapper.innerHTML = noCatMsg;
                return; 
            } 
            ...
            snapshot.forEach(childSnapshot => { 
                ...
                if (movieCatWrapper) movieCatWrapper.appendChild(createCheckbox(category.name));
                if (seriesCatWrapper) seriesCatWrapper.appendChild(createCheckbox(category.name));
            }); 
        }); 
    }
    ...
    async function updateCategoryName(id) {
        ...
        try {
            ...
            showStatus('Category renamed!', 'success');
            toggleCategoryEdit(id, false);
        } catch (err) {
    ...
    function populateMovieForm(data) { 
        ...
        document.getElementById('movie-rating').value = data.vote_average ? data.vote_average.toFixed(1) : ''; 
        document.getElementById('movie-storyline').value = data.overview; 
        
        const tmdbGenres = data.genres.map(g => g.name);
        document.querySelectorAll('#movie-category-wrapper input[type="checkbox"]').forEach(cb => {
            if (tmdbGenres.includes(cb.value)) {
                cb.checked = true;
            } else {
                cb.checked = false;
            }
        ...
        data.credits?.cast?.slice(0, 12).forEach(actor => addCastMember({ name: actor.name, photo: actor.profile_path ? `https://image.tmdb.org/t/p/w185${actor.profile_path}` : '' }, 'movie-cast-container')); 
    }
    ...
    function populateSeriesForm(data) { 
        ...
        document.getElementById('series-rating').value = data.vote_average ? data.vote_average.toFixed(1) : ''; 
        ...
        castContainer.innerHTML = ''; 
        data.credits?.cast?.slice(0, 12).forEach(actor => addCastMember({ name: actor.name, photo: actor.profile_path ? `https://image.tmdb.org/t/p/w185${actor.profile_path}` : '' }, 'series-cast-container')); 
    }
    ...
    function loadUsers() { 
        ...
        database.ref('users').on('value', snapshot => { 
            ...
            snapshot.forEach(childSnapshot => { 
                ...
                const trClass = isBanned ? 'banned-row' : '';
                const banLabel = isBanned ? '<span class="banned-badge">BANNED</span>' : '';
                const btnBanClass = isBanned ? 'unban-btn' : 'ban-btn';
                const btnBanText = isBanned ? '<i class="fa-solid fa-unlock"></i> Unban' : '<i class="fa-solid fa-ban"></i> Ban';
                ...
                tr.innerHTML = `
                    ...
                    <td>${user.email}</td>
                    <td style="color: var(--text-secondary); font-size: 12px;">${uid}</td>
                    <td>${isBanned ? '<span style="color:#c62828; font-weight:bold;">Banned</span>' : '<span style="color:#2e7d32; font-weight:bold;">Active</span>'}</td>
                    <td style="display:flex; gap:10px;">
                        <button class="btn-sm ${btnBanClass}" onclick="toggleUserBan('${uid}', ${isBanned})">${btnBanText}</button>
                        <button class="btn-icon" onclick="deleteUser('${uid}')" title="Delete User"><i class="fa-solid fa-trash"></i></button>
                    </td>
                `; 
    ...
    function toggleUserBan(uid, currentStatus) {
        const action = currentStatus ? "Unban" : "Ban";
        if (confirm(`Are you sure you want to ${action} this user?`)) {
            database.ref(`users/${uid}/isBanned`).set(!currentStatus)
            .then(() => showStatus(`User ${action}ned successfully!`, 'success'))
            .catch(err => showStatus(`Failed to ${action} user.`, 'error'));
        }
    }
    ...
    // NEW APP VERSION CONTROL FUNCTIONS
    ...
    function saveAppControlSettings() {
        const versionCode = parseInt(document.getElementById('app-ver-code').value);
        const versionName = document.getElementById('app-ver-name').value.trim();
        const forceUpdate = document.getElementById('app-force-update').checked;
        const apkUrl = document.getElementById('app-apk-url').value.trim();
        const changelog = document.getElementById('app-changelog').value.trim();

        if (!versionCode || !versionName) {
            showStatus('Version Code and Name are required.', 'error');
            return;
        }

        const data = { versionCode, versionName, forceUpdate, apkUrl, changelog };
        database.ref('app_settings/version').set(data)
        .then(() => showStatus('App Version Settings Updated!', 'success'))
        .catch(err => showStatus('Error saving settings: ' + err.message, 'error'));
    }

    // ==========================================
    // PREMIUM FEATURES LOGIC & ANALYTICS
    // ==========================================
    function savePlan() {
        const name = document.getElementById('plan-name').value.trim();
        const price = document.getElementById('plan-price').value.trim();
        const days = parseInt(document.getElementById('plan-days').value.trim());
        const devices = parseInt(document.getElementById('plan-devices').value.trim());
        const desc = document.getElementById('plan-desc').value.trim();

        if (!name || !price || !days || !devices || !desc) {
            showStatus('All plan fields are required.', 'error');
            return;
        }
        ...
        database.ref('plans').push(planData).then(() => {
            showStatus('Subscription Plan Added!', 'success');
        ...
        }).catch(err => showStatus('Error adding plan: ' + err.message, 'error'));
    }
        ...
        database.ref('plans').on('value', snapshot => {
            ...
            snapshot.forEach(child => {
                ...
                card.innerHTML = `
                    ...
                    <div class="details">
                        <span><i class="fa-solid fa-clock"></i> ${plan.days} Days Validity</span>
                        <span><i class="fa-solid fa-mobile-screen"></i> ${plan.devices} Device Limit</span>
    ...
    function approvePremium(reqKey, uid, days, planName, deviceLimit, price) {
        if (!confirm('Approve this payment and grant premium access?')) return;
        ...
        database.ref('users/' + uid + '/subscription').set(subData)
        .then(() => {
            return database.ref('premium_requests/' + reqKey).update({ 
                status: 'approved',
                approvedAt: Date.now(),
                revenueAmount: price 
            });
        ...
        .then(() => {
            return database.ref('notifications').push({
                ...
                message: `Your payment for ${planName} has been approved. Premium is now active until ${new Date(expiryDate).toLocaleDateString()}.`,
                icon: "fa-crown",
                target: "specific",
                targetEmails: [uid], 
                timestamp: Date.now()
        ...
        .then(() => {
            showStatus('Premium Granted Successfully!', 'success');
        })
    ...
    function handleManualPremium() {
        ...
        const days = parseInt(document.getElementById('manual-days').value.trim());

        if (!uid || !days) {
            showStatus('User UID and Days are required.', 'error');
            return;
        }
        ...
        database.ref('users/' + uid + '/subscription').set(subData)
        .then(() => {
            showStatus('Premium assigned manually to user!', 'success');
            document.getElementById('manual-uid').value = '';
            document.getElementById('manual-days').value = '';
        })
        .catch(err => showStatus('Failed to assign premium: ' + err.message, 'error'));
    }
    ...
    function loadRevenueAnalytics() {
        database.ref('users').on('value', snapshot => {
            ...
            snapshot.forEach(child => {
                ...
                if(sub && sub.isPremium && sub.expiryDate > now) {
                    vipCount++;
                }
            });
            document.getElementById('total-vip-stat').textContent = vipCount;
        });
        ...
        database.ref('premium_requests').orderByChild('status').equalTo('approved').on('value', snapshot => {
            ...
            snapshot.forEach(child => {
                const req = child.val();
                let priceStr = req.planPrice || req.revenueAmount || "0";
                let amount = parseFloat(priceStr.toString().replace(/[^0-9.]/g, ''));
                if(!isNaN(amount)) {
                    totalRevenue += amount;
                }
                if(priceStr.toString().includes('INR') || priceStr.toString().includes('₹')) currency = '₹';
            });
            
            document.getElementById('total-revenue-stat').textContent = currency + totalRevenue.toFixed(2);
        });
    }
    ...
    function savePaymentDetails() {
        ...
        const qrCodeUrl = document.getElementById('payment-qr-code').value.trim();
        ...
        database.ref('app_settings/payment').set(paymentData)
        .then(() => showStatus('Payment details saved successfully!', 'success'))
        .catch(err => showStatus('Error saving details: ' + err.message, 'error'));
    }
    ...
    function handleSaveAd() {
        ...
        const duration = parseInt(document.getElementById('ad-duration').value) || 5;
        const isActive = document.getElementById('ad-is-active').checked;

        if (!title || !mediaUrl) {
            showStatus('Ad Title and Media URL are required', 'error');
            return;
        }

        const adData = {
            title, type, mediaUrl, targetUrl, duration, isActive,
            timestamp: Date.now()
        };

        if (editId) {
            database.ref('ads/' + editId).update(adData)
            .then(() => {
                showStatus('Advertisement updated successfully', 'success');
                clearAdForm();
            })
            .catch(err => showStatus('Update failed: ' + err.message, 'error'));
        } else {
            database.ref('ads').push(adData)
            .then(() => {
                showStatus('Advertisement created successfully', 'success');
                clearAdForm();
            })
            .catch(err => showStatus('Create failed: ' + err.message, 'error'));
        }
    }
    ...
    function loadAds() {
        ...
        database.ref('ads').orderByChild('timestamp').on('value', snapshot => {
            container.innerHTML = '';
            if(!snapshot.exists()) {
                container.innerHTML = '<p style="color:var(--text-secondary)">No advertisements found.</p>';
                return;
            }
            ...
            snapshot.forEach(child => {
                ...
                let previewHtml = '';
                if(ad.type === 'video') {
                    previewHtml = `<video src="${ad.mediaUrl}" controlsList="nodownload" playsinline></video>`;
                } else {
                    previewHtml = `<img src="${ad.mediaUrl}" alt="${ad.title}">`;
                }
                ...
                card.innerHTML = `
                    ...
                    <div class="ad-preview">${previewHtml}</div>
                    <div class="ad-content">
                        <div class="ad-title">${ad.title}</div>
                        <div class="ad-meta">
                            <span><i class="fa-solid fa-clock"></i> ${ad.duration}s</span>
                            ${ad.targetUrl ? `<span><i class="fa-solid fa-link"></i> Linked</span>` : ''}
                        </div>
                    </div>
                    <div class="ad-controls">
                        <div class="switch-container">
                            <label class="switch">
                                <input type="checkbox" ${ad.isActive ? 'checked' : ''} onchange="toggleAdStatus('${key}', this.checked)">
                                <span class="slider"></span>
                            </label>
                        </div>
                        <div style="display:flex; gap:8px;">
                             <button class="btn-icon" onclick="editAd('${key}')" title="Edit"><i class="fa-solid fa-pencil"></i></button>
                             <button class="btn-icon" onclick="deleteData('ads', '${key}')" title="Delete"><i class="fa-solid fa-trash"></i></button>
                        </div>
                    </div>
                `;
    ...
    // ==========================================
    // UPDATED REDEEM CODES FUNCTIONS
    // ==========================================
    function generateRedeemCodes() {
        const days = parseInt(document.getElementById('redeem-days').value);
        const price = document.getElementById('redeem-price').value.trim(); // Get Price
        ...
        if (!price) {
            showStatus('Please enter a price for the code.', 'error');
            return;
        }
        if (!count || count < 1 || count > 50) {
            showStatus('Please enter a valid count (1-50).', 'error');
            return;
        }

        // Optional: Check user ID if available, otherwise default to 'Admin'
        // This prevents the error if admin isn't authenticated via a login page
        let creatorId = 'Admin';
        try {
            ...
            console.log("Not logged in, proceeding as Admin");
        }

        const updates = {};
        for (let i = 0; i < count; i++) {
             const code = generateRandomCode();
             updates[`/redeem_codes/${code}`] = {
                days: days,
        ...
        .then(() => {
            showStatus(`${count} code(s) generated successfully!`, 'success');
        ...
        .catch(err => showStatus('Error generating codes: ' + err.message, 'error'));
    }

    function generateRandomCode() {
        const chars = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789';
        let code = '';
        for (let i = 0; i < 10; i++) { // Increased length for security
    ...
    function loadRedeemCodes() {
        ...
        database.ref('redeem_codes').orderByChild('createdAt').on('value', snapshot => {
            tbody.innerHTML = '';
            if (!snapshot.exists()) {
                tbody.innerHTML = '<tr><td colspan="7" style="text-align: center;">No codes generated yet.</td></tr>';
                return;
            }
            ...
            snapshot.forEach(child => {
                ...
                const statusText = data.status === 'available' ? 'Available' : 'Used';
                const usedBy = data.usedBy ? data.usedBy : '-';
                const usedAt = data.usedAt ? new Date(data.usedAt).toLocaleString() : '-';
                const price = data.price || 'N/A'; // Get Price

                let actions = '';
                if (data.status === 'available') {
                    actions = `<button class="code-delete-btn" onclick="deleteRedeemCode('${code}')" title="Delete Code"><i class="fa-solid fa-trash"></i></button>`;
                } else {
                    actions = '-';
                }
    ...
    function deleteRedeemCode(code) {
        if (confirm('Are you sure you want to delete this unused code?')) {
            database.ref('redeem_codes/' + code).remove()
            .then(() => showStatus('Code deleted.', 'success'))
            .catch(err => showStatus('Error deleting code: ' + err.message, 'error'));
        }
    }
